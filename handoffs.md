...
- `.venv\Scripts\python.exe -m pytest` en `backend/`: 2 passed, 32 warnings.
- `.venv\Scripts\python.exe -m alembic upgrade head` en `backend/`: aplicado correctamente sobre SQLite.
- `rg "[ ]" issues`: sin criterios pendientes en los 14 archivos de issues.

Proximo contexto recomendado:
Revisar `architecture-checkpoint.md`, habilitar Git, confirmar `git status`/`git log`, y luego hacer commit/PR con mensaje por issue o squash final trazable.
