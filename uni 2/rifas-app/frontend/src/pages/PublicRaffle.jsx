...
export default function PublicRaffle({ token }) {
  ...
  async function reserve(event) {
    ...
    try {
      ...
      if (buyer.payment_method === "card" || buyer.payment_method === "pse") {
        const checkout = await request(`/r/${token}/reservations/${reservation.id}/checkout`, { method: "POST" });
        window.location.href = checkout.checkout_url;
        return;
      }
      setMessage("Numero reservado. Contacta al organizador para coordinar el pago en efectivo. Tienes 5 dias.");
      setSelected(null);
  ...
  if (!raffle) {
    return <main className="shell min-h-screen">{message || "Cargando rifa..."}</main>;
  }
  ...
  return (
    <main className="shell min-h-screen">
      ...
      {raffle.status === "active" && selected !== null && selectedState === "available" && (
        <section className="panel">
          ...
          <form onSubmit={reserve} className="form">
            ...
            <p className="muted">
              {buyer.payment_method === "cash"
                ? "Contacta al organizador para coordinar el pago. Tienes 5 dias."
                : "Te enviaremos al checkout de Wompi. Tienes 48 horas para completar el pago."}
            </p>
