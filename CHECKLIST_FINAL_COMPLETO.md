...
- [ ] Historial Git/Ralph verificado: pendiente porque Git no esta disponible en PATH y esta carpeta no expone `.git`.

## Validacion tecnica

- [x] `npm.cmd run build` en `frontend/`.
- [x] `npm.cmd audit --audit-level=moderate` en `frontend/` con 0 vulnerabilidades.
- [x] `pytest` en `backend/`: 2 passed, 32 warnings.
- [x] `alembic upgrade head` en `backend/`: aplicado correctamente sobre SQLite.
...
- Antes de entrega final en GitHub hay que instalar o habilitar Git, revisar `git status`/`git log` y crear commits trazables por issue.
