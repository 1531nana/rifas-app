...
- [x] `GET /health` retorna `{"status": "ok"}` desde el backend
- [x] El frontend carga en el navegador sin errores de consola
- [x] TailwindCSS esta activo (un elemento con clase Tailwind se renderiza correctamente)
- [x] Alembic puede correr migraciones contra una BD PostgreSQL local
- [x] Existe `.env.example` con todas las variables necesarias documentadas
